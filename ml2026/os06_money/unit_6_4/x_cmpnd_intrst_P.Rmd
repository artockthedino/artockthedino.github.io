...
Each year, the money actually grows by multiplying by $\left(1+\frac{`r r`}{`r n`}\right)^{`r n`}$, which equals `r (1+r/n)^n`, which is more than the $`r 1+r`$ you might expect from $r$. The **effective annual yield** is `r ((1+r/n)^n-1)*100`%.


In the US, credit cards will often state an APR, but compound monthly (or even daily!), so the [effective interest rate](https://en.wikipedia.org/wiki/Effective_interest_rate) is higher (if you allow the debt to linger).
