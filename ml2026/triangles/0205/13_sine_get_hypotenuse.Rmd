...
A right triangle has an angle of `r degt`°. The opposite leg has a length of `r oppt` meters. How long, in meters, is the hypotenuse?
...
text(0.16*cos(ang/2),0.16*sin(ang/2),paste0(degt,"°"),col="red",cex=0.8)
```
\
...
$$\sin(`r degt`) = \frac{`r oppt`}{x}$$
Multiply both sides by $x$ and divide both sides by $\sin(`r degt`)$ to solve for $x$.

$$x = \frac{`r oppt`}{\sin(`r degt`)}$$
To evaluate $x$, type the following into a modern calculator: ``round(`r oppt`/sin(`r degt`))``. Make sure to be in degree mode, not radian mode.
