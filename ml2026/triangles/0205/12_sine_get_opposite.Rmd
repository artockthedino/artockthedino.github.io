...
A right triangle has an angle of `r degt`°. The hypotenuse has a length of `r hypt` meters. How long, in meters, is the leg opposite the angle?
...
text(0.16*cos(ang/2),0.16*sin(ang/2),paste0(deg,"°"),col="red",cex=0.8)
```
\
...
$$\sin(`r degt`) = \frac{x}{`r hypt`}$$
Multiply both sides by `r hypt` to solve for $x$.

$$`r hypt` \cdot \sin(`r degt`) = x$$
To evaluate $x$, type the following into a modern calculator: ``round(`r hypt`*sin(`r degt`))``. Make sure to be in **degree mode**, not radian mode.
