...
In the example below, a turtle draws a segment 1 unit long, turns 72° clockwise, draws a segment 2 units long, turns 72°,...and the segments get larger and larger up to 16 units, after which the next segment is 1 unit.
...
3. Make some variables.
    ...
    * **Make 7 variables**:
        * \color{red}`i`\color{black}:
        * \color{red}`i`\color{black}: the current counting number. It starts at 1, increases by 1 after each segment, and returns to 1 after reaching `n`. This `i` variable will change frequently during the course of producing the pattern. Each segment's length is the product of `i` times `scale`. The other variables are the "parameters": they will be set before the drawing begins, and not change during the drawing. This one (`i`) will change over and over while the code runs. The other variables acting this way are already defined by Scratch: `x position`, `y position`, and `direction`.
        ...
        * \color{red}`turn`\color{black}: angle turned after each segment (72 in the example above)
    * We want all the parameters to show on the screen, so keep them checked. Uncheck `i`.\
...
10. **DOCUMENT** your own patterns in a slideshow!
    ...
    * Try to center the image horizontally by adjusting the `x_inital` value.
