...
* Rules:
    ...
    * `x` $\to$ `Ax`
    * `-` $\to$ `-`
(Notice that since "-" is replaced with "-", it is a **constant**. We usually do not list constants in the rules; constants are implied by not having a rule.)
* Find the recursions (the iterations):
...
plotit2 = function(xyd,lwd=1,col="blue"){
    ...
    for(xxx in -8:8){
        jjj = -6:6
        points(rep(xxx,length(jjj)),jjj,pch=19,col="grey")
    }
    starts = numeric()
    if(d[1]){
        starts = c(1)
...
for(i in -5:5){
    ds = -5:5
    points(rep(i,length(ds)),ds,pch=20)
}
points(0,0,pch=8)
```
...
* Use keyboard buttons to control turtle using the following moves:
    * Move forward
    * Turn right
    * Turn left
    * Hop forward (and do not draw)
    * Save position at top of list
    * Return to top-of-list position and delete position from list
* Copy and paste a string of characters (a "word") into Scratch, each character corresponding to a move; turtle draws whole picture.
    * We can use [a L-system string generator](https://chadworley.github.io/pbl/L-systems/Lsys_string_gen.html) to create the strings.
