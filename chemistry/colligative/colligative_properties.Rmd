...
Since pure water freezes at 0°C, we estimate this solution freezes at `r signif(-2*1.86*m,3)`°C.
3. Calculate boiling point.
$$\Delta T_\text{b} = 2 \cdot 0.512 \cdot `r m` = `r signif(2*0.512*m,3)` ~^\circ \mathrm{C}$$
Since pure water boils at 100°C, we estimate this solution boils at `r 100+signif(2*0.512*m,3)`°C.
