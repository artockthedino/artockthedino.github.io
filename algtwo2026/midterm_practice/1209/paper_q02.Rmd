...
## Midterm practice: Transform squaring function
...
while(length(handles)<9){
    ...
    h = hk[1]
    k = hk[2]
    isHS = sample(c(T,F),1,F,c(0.3,0.7))
    if(isHS){
        a = 1
        b = sample(2:3,1)
        expr = paste0("y=\\left(\\frac{1}{",b,"}(",pn(c(1,-h),1:0),")\\right)^2+",k,collapse="")
        expr = gsub("+-","-",expr,fixed=T)
        if(b<0){
            step1 = paste0("Horizontal stretch by factor ",abs(b),", and horizontal reflection.")
        }
        else{
            step1 = paste0("Horizontal stretch by factor ",abs(b),".")
        }
    } else {
        a = sample(2:3,1)*sample(c(-1,1),1)
        b = 1
        expr = paste0("y=",a,"\\left(",pn(c(1,-h),1:0),"\\right)^2+",k,collapse="")
        expr = gsub("+-","-",expr,fixed=T)
        if(a<0){
            step1 = paste0("Vertical stretch by factor ",abs(a),", and vertical reflection.")
        }
        else{
            step1 = paste0("Vertical stretch by factor ",abs(a),".")
        }
    }
    x = seq(-12,12,0.01)
    y = a*(1/b*(x-h))^2+k
...
*On the actual midterm, the helpful dots will not be shown.*
...
for(n in 1:9){
    ...
    points(hs[n]+bs[n],ks[n]+as[n],pch=19,col="blue",cex=0.8)
    points(hs[n]+2*bs[n],ks[n]+4*as[n],pch=19,col="blue",cex=0.8)
    points(hs[n]+3*bs[n],ks[n]+9*as[n],pch=19,col="blue",cex=0.8)
    points(hs[n]-bs[n],ks[n]+as[n],pch=19,col="blue",cex=0.8)
    points(hs[n]-2*bs[n],ks[n]+4*as[n],pch=19,col="blue",cex=0.8)
    points(hs[n]-3*bs[n],ks[n]+9*as[n],pch=19,col="blue",cex=0.8)
    points(hs[n],ks[n],pch=19,col="blue",cex=0.8)
