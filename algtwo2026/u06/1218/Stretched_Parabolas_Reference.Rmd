...
Consider the stretched parabola $y=ax^2$, where $a$ is any constant. For the table below, some simple positive values of $a$ were used. When the output is a whole number less than 16, it is highlighted in red, because those lattice points are indicated as dots in the graphs on the back of this page.
...
for(i in 1:length(as)){
    ...
    for(j in 1:length(x)){
        y = as[i]^ps[i]*x[j]^2
        if(y<=15 && y%%1==0){
            colu = c(colu,paste0("\\color{red}",fractions(as[i]^ps[i]*x[j]^2),"\\color{black}",collapse=""))
        } else {
...
That's a lot of information. I think it is easier to appreciate the graphs (see back).
